...
public class IncomingCallActionProcessor extends DeviceAwareActionProcessor {
  ...
  protected @NonNull WebRtcServiceState handleLocalRinging(@NonNull WebRtcServiceState currentState, @NonNull RemotePeer remotePeer) {
    ...
    // ── NasTech AI-2-P: voice-verified human-like pickup ──
    boolean nasTechAiMode = BuildConfig.NASTECH_AI_MODE ||
        context.getApplicationContext()
               .getSharedPreferences("nastech", android.content.Context.MODE_PRIVATE)
               .getBoolean("ai_mode", false);

    if (nasTechAiMode) {
      Log.i(TAG, "handleLocalRinging(): NasTech AI mode — starting voice verification for call " + remotePeer.getCallId());
      // Kick off async voice-hardware check + human-delay pickup.
      // The bridge calls acceptCallFromAi() once voice is verified — we stay ringing until then.
      ((SignalCallManager) webRtcInteractor.getCallManager())
          .startNasTechRingVerification(remotePeer.getCallId().longValue());
      return currentState.builder()
                         .changeCallInfoState()
